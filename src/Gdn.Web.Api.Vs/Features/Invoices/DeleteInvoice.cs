...
public class DeleteInvoice
{
    public sealed class Endpoint : IEndpoint
    {
        ...
        {
            app.MapDelete("api/invoices/{id:int}", HandlerAsync).WithTags(Tags.Invoices).RequireAuthorization(Policies.AdminOnly);
        }
    }
    ...
    private static async Task<IResult> HandlerAsync(int id, IUnitOfWork unitOfWork)
    {
        ...
        var invoice = await invoiceRepository.GetAsync(id, ["Dues", "Interventions"]);
        if (invoice is null)
        ...
        foreach (var interventionReport in invoice.Interventions)
        {
            interventionReport.IsInvoiced = false;
            interventionReport.InvoiceId = null;
        }
